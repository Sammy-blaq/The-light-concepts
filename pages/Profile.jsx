...
const Profile = () => {
  // SET AND TRACK THE ACTIVE ACTION BUTTON
  const [btnActive, setBtnActive] = useState(0);
  const buttonsText = ["Account", "Order", "Booking", "Complaint"];

  // Content according to active button
      ...
      <main className="main-content flex flex-col  flex-1 " id="top">
          ...
          <div className="flex items-center flex-col gap-8 text-center sm:text-left sm:flex-row sm:gap-4 mx-6">
            ...
            <div className="flex flex-col gap-10">
              <h1 className="text-[1.6rem] sm:text-[2.6rem]  text-[#504230] font-bold">
                Welcome, <span className="font-normal">John Doe...</span>
              </h1>
              <p className=" text-[#8a775c]">you@example.com</p>
            </div>
          </div>
        </section>
        <section className="container pt-24 mx-30">
          <div className="grid grid-cols-2 gap-4 sm:gap-0 sm:grid-cols-4 bg-[#ededd4] p-4 rounded-xl mb-8 mx-6">
            {/* iterating throuth the buttonsText to display label and handle state */}
