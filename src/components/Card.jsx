...
const Card = ({ title, icon, subheading, text, link, lintText, linkIcon }) => {
  return (
    <div className="flex flex-col gap-8 bg-background-accent py-8 px-8 rounded-lg border border-border shadow-sm hover:shadow-xl transition-shadow duration-300">
      ...
      <div class="flex">
        <Link
          to={link}
          className="flex items-center gap-4 p-2 text-[1.4rem] text-text-foreground hover:underline"
        >
          {lintText} {linkIcon}
        </Link>
      </div>
